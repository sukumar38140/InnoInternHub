...
export default function ProjectPage({ params }: ProjectPageProps) {
  ...
  useEffect(() => {
    ...
    params.then(async (p) => {
      setId(p.id);
      try {
        const found = await projectsApi.get(p.id);
        setProject(found);
      } catch (error) {
  ...
  if (!project) {
    return (
      <div className={cn("min-h-screen bg-slate-50", isAuthenticated ? "flex" : "")}>
        {isAuthenticated ? <Sidebar /> : <Navbar />}
        <main className={cn(isAuthenticated ? "flex-1 ml-64 flex flex-col min-h-screen" : "")}>
        <div className={cn("pb-20", isAuthenticated ? "pt-8" : "pt-24")}>
              ...
              <Card className="p-6 bg-white border-slate-200 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-sm">
                    {project.innovator ? `${project.innovator.firstName[0]}${project.innovator.lastName[0]}`.toUpperCase() : "I"}
                  </div>
                  <div>
                    <p className="font-medium text-slate-900">
                      {project.innovator ? `${project.innovator.firstName} ${project.innovator.lastName}` : "Innovator"}
                    </p>
                    {project.innovator?.companyName && (
                      <p className="text-xs text-slate-500 mt-0.5">{project.innovator.companyName}</p>
                    )}
                    {project.innovator?.isVerified && (
                      <span className="inline-flex items-center text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-full mt-1.5 border border-green-100">
