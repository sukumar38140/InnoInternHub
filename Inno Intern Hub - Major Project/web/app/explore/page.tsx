...
function mapProjectToIdea(project: Project): Idea {
    return {
        ...
        deadline: project.applicationDeadline ? new Date(project.applicationDeadline) : undefined,
        requirements: [],
        teamSize: project.teamSize,
        applicationsCount: project.applicationCount || 0,
        viewsCount: project.viewCount || 0,
        innovator: {
            id: project.innovator?.id || "",
            name: project.innovator
                ? `${project.innovator.firstName} ${project.innovator.lastName}`
                : "Innovator",
            verified: project.innovator?.isVerified ?? true
        }
    };
}
...
export default function ExplorePage() {
    ...
    useEffect(() => {
        const loadIdeas = async () => {
            try {
                const response = await projectsApi.list({ status: "OPEN" });
                const mappedIdeas = response.projects.map(mapProjectToIdea);
    ...
    return (
        <div className={cn("min-h-screen bg-slate-50", isAuthenticated ? "flex" : "")}>
            ...
            <main className={cn(isAuthenticated ? "flex-1 ml-64 flex flex-col min-h-screen" : "")}>
                {/* Search Header */}
                <div className={cn("pb-8 bg-white border-b border-slate-200", isAuthenticated ? "pt-8" : "pt-24")}>
