...
function StudentDashboard() {
    ...
    useEffect(() => {
        const loadDashboard = async () => {
            try {
                const apiStats = await userApi.getStats();
                const appsMap = apiStats.applications || {};
                const activeApplications = (appsMap.PENDING || 0) + (appsMap.SHORTLISTED || 0) + (appsMap.ACCEPTED || 0);

                setStats({
                    activeApplications,
                    completedProjects: apiStats.certificates || 0,
                    innovationPoints: apiStats.points || 0,
                    currentLevel: apiStats.level || "Beginner",
                    pendingReviews: 0,
                });
            } catch (error) {
