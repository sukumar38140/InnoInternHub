...
if (typeof window !== "undefined") {
    accessToken = localStorage.getItem("innohub_access_token");
}
...
export async function api<T>(endpoint: string, options: ApiOptions = {}): Promise<T> {
    ...
    // Refresh token from storage if needed
    if (typeof window !== "undefined" && !accessToken) {
        const storedToken = localStorage.getItem("innohub_access_token");
        if (storedToken) {
        ...
        if (response.status === 401 && requireAuth) {
            ...
            if (refreshed && accessToken) {
                ...
                if (!retryResponse.ok) {
                    ...
                    throw new ApiError(
                        error.error || "Request failed after token refresh",
            ...
            if (typeof window !== "undefined") {
                localStorage.removeItem("innohub_access_token");
                localStorage.removeItem("innohub_user");
